...
LTNode* LoopTree::build_loop_tree(std::vector<LTNode*>& nodes) {
    ...
    for (auto& node : nodes) {
        ...
        for (auto& potential_parent : nodes) {
            ...
            if (std::find(parent_blocks.begin(), parent_blocks.end(), node->get_header_id()) != parent_blocks.end()) {
...
std::vector<LTNode*> LoopTree::build_loop_vector(Graph* graph) {
    ...
    for (auto& loop : loops) {
        ...
        for (auto& cur_latch : latches) {
            ...
            for (const auto& elem : rpo_ids) { looped_ids.insert(elem); }
    ...
    std::copy_if(all_ids.begin(), all_ids.end(), std::back_inserter(non_looped_ids),
