...
TEST(LoopAnalyzerTest, FirstExample) {
  ...
  std::vector<size_t> expected_blocks = {
      L->get_id(), V->get_id(), D->get_id(), Q->get_id(), T->get_id(), G->get_id()};
  ...
  auto all_latches_found =
      std::all_of(expected_latches.begin(), expected_latches.end(), [&real_latches](int a_elt) {
        return std::find(real_latches.begin(), real_latches.end(), a_elt) != real_latches.end();
      });
  ASSERT_TRUE(all_latches_found);

  auto all_blocks_found =
      std::all_of(expected_blocks.begin(), expected_blocks.end(), [&real_blocks](int a_elt) {
...
TEST(LoopAnalyzerTest, SecondExample) {
  ...
  std::vector<std::size_t> expected = {
      A->get_id(), B->get_id(), C->get_id(), D->get_id(), E->get_id(), F->get_id(), G->get_id()};
  auto each_block = std::all_of(expected.begin(), expected.end(), [&real](int a_elt) {
    return std::find(real.begin(), real.end(), a_elt) != real.end();
  });
  ASSERT_TRUE(each_block);
...
TEST(LoopAnalyzerTest, ThirdExample) {
  ...
  auto each_block = std::all_of(
      root_blocks_expected.begin(), root_blocks_expected.end(), [&root_blocks_real](int a_elt) {
        return std::find(root_blocks_real.begin(), root_blocks_real.end(), a_elt) !=
               root_blocks_real.end();
      });
  ASSERT_TRUE(each_block);
...
TEST(LoopAnalyzerTest, FifthExample) {
  ...
  auto root_each_block = std::all_of(
      root_blocks_expected.begin(), root_blocks_expected.end(), [&root_blocks_real](int a_elt) {
        return std::find(root_blocks_real.begin(), root_blocks_real.end(), a_elt) !=
               root_blocks_real.end();
      });
  ...
  auto each_block =
      std::all_of(blocks_expected.begin(), blocks_expected.end(), [&blocks_real](int a_elt) {
